...
def handler(event, context):
    ...
    try:    
        ...
        for j in jobs:
            ...
            logger.info("Hitting file endpoint")
            
            try:
                logger.info("trying to HTTP connect")
                #splitting the source name to file name
                split_by_ext = sourceS3.rsplit(sep='.', maxsplit=1)
                just_the_file_name = split_by_ext[0].rsplit(sep='/',maxsplit=1)[1]
                #can be move to environment variable
                url = 'http://'+str(ec2)+'.compute-1.amazonaws.com:8080/natyaguru/v1/catalog/file/{}'.format(str(just_the_file_name))
                requests.put(url)
